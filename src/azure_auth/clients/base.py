...
The synchronous clients in ``azure_auth._sync`` are generated from this package by
``scripts/generate_sync.py``. Keep asynchronous constructs plain (``async def``, ``await``,
``async with``, ``async for``, ``asyncio.sleep``) so the generator can translate them.
"""
