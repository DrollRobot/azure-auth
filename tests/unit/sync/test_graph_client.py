# Generated by scripts/generate_sync.py from tests/unit/test_graph_client.py.
...
def sleeps(monkeypatch: pytest.MonkeyPatch) -> list[float]:
    ...
    monkeypatch.setattr("azure_auth._sync.base.time.sleep", fake_sleep)
    return delays
...
def test_request_carries_a_token_for_the_graph_defaults(
    ...
    with GraphClient(
        ...
        body = graph.get("/me", params={"$select": "id"})
...
def test_client_id_precedence(fake_msal: FakeMsal) -> None:
...
def test_login_uses_the_clients_id_and_scopes(auth: AuthContext, fake_msal: FakeMsal) -> None:
...
def test_write_verbs_send_json_and_tolerate_empty_responses(auth: AuthContext) -> None:
    ...
    created = graph.post("/users", {"displayName": "A"})
    patched = graph.patch("/users/1", {"displayName": "B"}, headers={"If-Match": "*"})
    replaced = graph.put("/users/1/photo", {"x": 1})
    deleted = graph.delete("/users/1")
...
def test_error_response_becomes_graph_error(auth: AuthContext) -> None:
...
def test_non_json_error_keeps_the_text(auth: AuthContext) -> None:
    graph = GraphClient(
...
def test_oauth_style_error_is_understood(auth: AuthContext) -> None:
...
def test_throttled_request_waits_for_retry_after(auth: AuthContext, sleeps: list[float]) -> None:
    recorder = Recorder(
        [ok(status=429, headers={"Retry-After": "7"}), ok(status=503), ok({"ok": 1})]
    )
    graph = GraphClient(auth, transport=recorder.transport)

    assert graph.get("/users") == {"ok": 1}
    assert sleeps == [7.0, 4.0]
    assert len(recorder.requests) == 3


def test_throttling_gives_up_after_max_retries(auth: AuthContext, sleeps: list[float]) -> None:
...
def test_claims_challenge_is_passed_to_msal(auth: AuthContext, fake_msal: FakeMsal) -> None:
...
def test_plain_401_forces_one_token_refresh(auth: AuthContext, fake_msal: FakeMsal) -> None:
    fake_msal.accounts = [{"username": USER}]
    fake_msal.silent = lambda call: token_result()
...
def test_get_all_follows_next_links(auth: AuthContext) -> None:
    next_link = f"{GRAPH}/users?$skiptoken=abc"
    recorder = Recorder([ok({"value": [1, 2], "@odata.nextLink": next_link}), ok({"value": [3]})])
    graph = GraphClient(auth, transport=recorder.transport)

    items = graph.get_all("/users", params={"$top": 2}, headers={"ConsistencyLevel": "eventual"})
...
def test_token_is_never_sent_to_another_host(auth: AuthContext, url: str) -> None:
...
def test_batch_is_chunked_and_order_is_restored(auth: AuthContext) -> None:
...
def test_customer_tenants_come_from_delegated_admin_customers(auth: AuthContext) -> None:
    recorder = Recorder(
        [ok({"value": [{"tenantId": "t1", "displayName": "A"}, {"tenantId": "t2"}]})]
    )
    graph = GraphClient(auth, transport=recorder.transport)

    assert graph.list_customer_tenant_ids() == ["t1", "t2"]
