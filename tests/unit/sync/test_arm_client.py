# Generated by scripts/generate_sync.py from tests/unit/test_arm_client.py.
...
def sleeps(monkeypatch: pytest.MonkeyPatch) -> list[float]:
    ...
    monkeypatch.setattr("azure_auth._sync.arm.time.sleep", fake_sleep)
    return delays


def test_api_version_is_added_to_every_verb(auth: AuthContext, fake_msal: FakeMsal) -> None:
    recorder = Recorder(lambda request: ok({"name": "rg"}))
    arm = AzureClient(auth, transport=recorder.transport)

    arm.get(GROUP, api_version="2021-04-01", params={"$expand": "x"})
    arm.put(GROUP, {"location": "westeurope"}, api_version="2021-04-01")
    arm.patch(GROUP, {"tags": {}}, api_version="2021-04-01")
    arm.post(f"{GROUP}/validate", api_version="2021-04-01")
    arm.delete(GROUP, api_version="2021-04-01")
...
def test_get_all_follows_next_link_as_given(auth: AuthContext) -> None:
    ...
    assert arm.get_all("/subscriptions", api_version="2022-12-01") == ["a", "b"]
    assert recorder.requests[1].url.params.get_list("api-version") == ["2022-12-01"]


def test_error_becomes_azure_error(auth: AuthContext) -> None:
    ...
    with pytest.raises(AzureError) as caught:
        arm.get(GROUP, api_version="2021-04-01")
...
def test_put_polls_the_operation_then_reads_the_resource(
    ...
    started = arm.send("PUT", GROUP, api_version="2021-04-01", json={"location": "x"})
    result = arm.wait(started)
...
def test_post_operation_returns_the_location_result(auth: AuthContext, sleeps: list[float]) -> None:
    ...
    started = arm.send("POST", f"{GROUP}/export", api_version="2021-04-01")

    assert arm.wait(started, poll_interval=0.5) == {"output": 42}
    assert sleeps == [0.5]
    assert recorder.urls()[-1] == location


def test_operation_without_location_returns_its_status(
    auth: AuthContext, sleeps: list[float]
) -> None:
    recorder = Recorder(
        [
    ...
    started = arm.send("DELETE", GROUP, api_version="2021-04-01")

    assert arm.wait(started) == {"status": "Succeeded", "properties": {"x": 1}}
    assert sleeps == [5.0]


@pytest.mark.parametrize("state", ["Failed", "Canceled"])
def test_failed_operation_raises(auth: AuthContext, sleeps: list[float], state: str) -> None:
...
def test_location_only_operation_is_polled_until_done(
    auth: AuthContext, sleeps: list[float]
) -> None:
    ...
    assert arm.wait(started) is None
    assert sleeps == [5.0, 2.0]


def test_completed_response_is_returned_as_is(auth: AuthContext, sleeps: list[float]) -> None:
    arm = AzureClient(auth, transport=Recorder([ok({"done": True})]).transport)
    started = arm.send("PUT", GROUP, api_version="2021-04-01")
    assert arm.wait(started) == {"done": True}
    assert sleeps == []


def test_wait_times_out(auth: AuthContext, sleeps: list[float]) -> None:
    recorder = Recorder(
    ...
    with pytest.raises(TimeoutError, match="still running"):
        arm.wait(started, timeout=-1)


def test_operation_url_on_another_host_is_refused(auth: AuthContext, sleeps: list[float]) -> None:
    recorder = Recorder(
