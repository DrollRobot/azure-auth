...
async def test_the_cmdlets_own_reason_leads_the_error_message(auth: AuthContext) -> None:
...
async def test_an_unknown_cmdlet_is_an_error_and_not_a_decoding_failure(
    auth: AuthContext,
) -> None:
...
async def test_a_blank_403_body_is_explained(auth: AuthContext) -> None:
    exchange = ExchangeClient(
...
async def test_an_undecodable_success_is_still_an_error(auth: AuthContext) -> None:
...
async def test_warnings_are_collected_from_the_header_and_the_body(auth: AuthContext) -> None:
    pages = [
        ok(
            {
                "value": [{"n": 1}],
